...
namespace DCB.Db.Repository
{
    ...
    public class Repository<T> : IRepository<T> where T : class, IEntity 
    {
        ...
        public async Task<DB.Model.PagedResult<T>> GetAsync(Filter<T> filter, CancellationToken token)
        {
        ...
        /// <param name="token">токен</param>
        /// <returns>список моделей</returns>
        public async Task<DB.Model.PagedResult<T>> GetAsyncDeleted(Filter<T> filter, CancellationToken token)
        {
            return await GetAsyncInternal(filter, true, "GetAsyncDeleted");
        }

        private async Task<DB.Model.PagedResult<T>> GetAsyncInternal(Filter<T> filter, bool withDeleted, string methodName)
        {
            return await ExecuteAsync(async (context) =>
            {
                var pageCount = 1;
                ...
                return new DB.Model.PagedResult<T>(result, pageCount);
            }, methodName, false);
        }
